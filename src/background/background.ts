...
const saveConfigProxy = async (request) => {
  ...
  const config = {
    ...
    rules: {
      ...
      bypassList: ["192.168.*.*, localhost ,127.0.0.1"]
    }
  };
...
const checkProxy = async () => {
  try {
    const response = await fetch('https://worker.goprofilev2.net/ip');
    if (!response.ok) throw new Error('Failed to fetch proxy status');
    const result = await response.json();
    console.log(result);
    return result;
  } catch (error) {
    console.error("Error when checkProxy:", error);
    return null;
  }
};
...
chrome.webRequest.onErrorOccurred.addListener(
  async function (details) {
    if (details.error) {
      ...
      if (!isProxy) {
        await handleFetchNewIp();
      } else {
        console.log("Proxy is still working", isProxy);
      }
    }
  },
  { urls: ["<all_urls>"] }
